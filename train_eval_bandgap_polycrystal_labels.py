# src/train_eval_bandgap_polycrystal_labels.py
import torch
...
def bandgap_polycrystal_plot(model_class, df, graphs, model_name="GCN",
    ...
    results.to_csv(f"results/{model_name}_bandgap_polycrystal_labels.csv", index=False)

    # Plot
    plt.figure(figsize=(10,7))

    # Scatter inorganic compounds
    ...
    plt.plot(x_vals, fom2, color="blue", linestyle="--", label="Figure of Merit 2")

    # Highlight specific compounds with annotation lines
    highlight = {
        "CoSb3": (1.5, 15),   # Example values, replace with actual
        "HfO2": (5.8, 25),
        "SiO2": (9.0, 3.9),
        "Polyethylene": (6.5, 2.3)
    }
    for formula, (eg, diel) in highlight.items():
        plt.scatter(eg, diel, color="black", s=120, marker="*", label=formula)
        plt.annotate(formula,
                     xy=(eg, diel),
                     xytext=(eg+1.0, diel+5),
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"results/{model_name}_bandgap_polycrystal_labels.png", dpi=1200, bbox_inches="tight")
    plt.close()
